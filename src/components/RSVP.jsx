...
export default function RSVP() {
  ...
  const selectGuest = async (guest) => {
    ...
    try {
      ...
      if (!snap.exists()) {
        showNotification('Invité introuvable. Veuillez contacter les mariés. / Guest not found.', 'error');
        return;
      }
      ...
      showNotification('Erreur de chargement. Veuillez réessayer. / Could not load guest.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitRSVP = async (attending) => {
    ...
    try {
      ...
      showNotification('Échec de la soumission. Veuillez réessayer. / Failed to submit RSVP.', 'error');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="rsvp__wrap">
            ...
            <div className="rsvp__instructions">
              ...
              <ol className="rsvp__steps">
                <li>
                  <span className="step-fr">Recherchez votre nom tel qu'il figure sur votre invitation.</span>
                  <span className="step-en">Search for your name exactly as written on your invitation.</span>
                </li>
                <li>
                  <span className="step-fr">Sélectionnez votre nom dans la liste.</span>
                  <span className="step-en">Select your name from the list.</span>
                </li>
                <li>
                  <span className="step-fr">Confirmez votre présence ou votre absence.</span>
                  <span className="step-en">Confirm whether you will be attending.</span>
                </li>
              </ol>
                    ...
                    <ul className="rsvp__results">
                      {results.length === 0 ? (
                        <li className="rsvp__no-result">
                          <span className="step-fr">Aucun résultat. Vérifiez l'orthographe ou contactez les mariés.</span>
                          <span className="step-en">No match found. Check spelling or contact the couple.</span>
                        </li>
                      ) : (
                        results.map(g => (
                          <li key={g.id} className="rsvp__result-item" onClick={() => selectGuest(g)}>
                            {g.name}
                            {g.partySize > 1 && (
                ...
                {submittedYes ? (
                  <>
                    ...
                    <div className="rsvp__success-sub">
                      <p className="rs-fr">Votre confirmation a été enregistrée. Nous avons hâte de célébrer avec vous le 03 octobre 2026.</p>
                      <p className="rs-en">Your RSVP has been recorded. We can't wait to celebrate with you on 03 October 2026.</p>
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div className="rsvp__success-sub">
                      <p className="rs-fr">Nous sommes désolés que vous ne puissiez pas être des nôtres. Vous serez dans nos pensées ce jour-là.</p>
                      <p className="rs-en">We're sorry you won't be able to make it. You'll be in our thoughts on the day.</p>
                    </div>
                  </>
                )}
                <button
